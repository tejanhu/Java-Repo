...
public class et_pet_alien {
	...
	JOptionPane.showMessageDialog(null, aryNums[3] );
		...
		System.out.println("THE END.");
			...
			public static alienkids(int aryNums [])
		{


			String rascals [] = {"alex","alie","xela","polo","yoshi"};
			int [] aryNums = Integer.parse int [] (String [] rascals); 
			String FEED;

		FEED = JOptionPane.showMessageDialog("It's feeding time!");			


	   for (int i=0; i<=aryNums;i++)
	{

		if (aryNums[i]>aryNums[4])
	{    swap (aryNums,i,4);

	} // END if

		else (aryNums[i]<aryNums[4])
	{    swap (aryNums,4,i);
	} // END else
