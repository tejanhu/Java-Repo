...
class chatbot6
{
	final static int value=6;//final variable which has one value alone, which is static so it can be used throughout multiple methods without being passed into		
    	...
    	public static void main (String[] param)//Runs main method and runs all other methods, as well as declaring a loop for users and declares and intials multiple strings
    	{
	...
	//Asks for basic facts for user e.g.name and favourite team and stored using String
	{
      		...
      		userfact1 = JOptionPane.showInputDialog("I'm glad to hear it " + userfact_i + ". Do you like Football? ");//Here are the first set of decision's, which give out a response depending on what the user previously inputs.
